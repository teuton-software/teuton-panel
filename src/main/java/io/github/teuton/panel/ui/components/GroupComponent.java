...
import io.github.teuton.panel.ui.model.cases.Test;
...
public class GroupComponent extends Accordion implements Initializable {
	...
	private void onGroupsChanged(Change<? extends Group> changes) {
		...
		while (changes.next()) {
			changes.getAddedSubList().stream().forEach(group -> {
				TargetComponent groupComponent = new TargetComponent();
